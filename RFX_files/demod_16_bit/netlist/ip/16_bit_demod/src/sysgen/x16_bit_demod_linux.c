...
typedef struct {
    ...
    x16_bit_demod_uio_map maps[ MAX_UIO_MAPS ];
} x16_bit_demod_uio_info;

/***************** Variable Definitions **************************************/
static x16_bit_demod_uio_info uio_info;
...
static int uio_info_read_name(x16_bit_demod_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(x16_bit_demod_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(x16_bit_demod_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(x16_bit_demod_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int x16_bit_demod_Initialize(x16_bit_demod *InstancePtr, const char* InstanceName) {
 x16_bit_demod_uio_info *InfoPtr = &uio_info;
